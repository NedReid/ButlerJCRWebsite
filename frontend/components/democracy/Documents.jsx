...
class DocumentsComponent extends React.Component {
    ...
    render() {
        ...
        for (let documentEnumValue in Object.values(documentEnum)) {
            let enumDoc = this.state.documents.filter((document) => {
                return document.category == documentEnumValue;
            });
            enumDoc.sort((a, b) => {
            ...
            enumDoc.forEach((document) => {
                if (
                    enumDocByYear.length === 0 ||
                    enumDocByYear[enumDocByYear.length - 1][0].date.getFullYear() !==
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <Collapse.Content className="bg-white border-l-4 border-gray-200">
                                            <div>
                                                {documentsByCategory[index].map(
                                                    (yearDocument, yearDocumentIndex) => {
                                                        return (
                                                            <div
                                                                key={`${index}_${yearDocumentIndex}`}
                                                                className="my-1"
                                                            >
                                                                <div className="font-semibold text-lg my-1">
                                                                    {yearDocument[0].date.getFullYear()}
                                                                </div>
                                                                {yearDocument.map(
                                                                    (document, documentIndex) => {
                                                                        return (
                                                                            <div
                                                                                key={`${index}_${yearDocumentIndex}_${documentIndex}`}
                                                                            >
                                                                                <button
