...
class FAQ extends React.Component {
    ...
    async componentDidMount() {
        ...
        const res1 = res.map((q) => {
            return new FAQModel(q);
        });
        const res2 = res.map((q) => {
            return new FAQModel(q);
        });
        res1.sort((a, b) => {
    ...
    onChange = async () => {
        ...
        if (this.state.editing) {
            ...
            for (const nq of newFAQ) {
                let oq = this.state.originalFAQ.filter((q) => {
                    return nq._id === q._id;
                });
                if (oq.length === 0) {
                    console.log("NQ", nq);
            ...
            <div>
                ...
                this.state.saving === false ? (
                    <div className="relative m-4">
                        ...
                        <div className="mt-2">
                            JCR roles, motions and elections can be slightly complicated, and you
                            may have a lot of questions about how they are run! Here are a few
                            frequently asked questions: If you have any more questions, please
                            contact the JCR Chair!
                        </div>
                        ...
                        {this.state.FAQ.map((q, index) => {
                            return (
                                <FAQBlock
                                    q={q}
                                    index={index}
                                    last={index === this.state.FAQ.length - 1}
