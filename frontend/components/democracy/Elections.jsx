...
class ElectionsComponent extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            currentMotions: undefined,
            meetingHeaders: undefined,
    ...
    async componentDidMount() {
        ...
        this.setState({
            currentMeeting: new meetingModel(res.meeting),
            currentCandidates: res.candidates,
            currentRoles: res.roles,
            currentMotions: res.motions,
            meetingHeaders: meetingHeaders,
        });
    ...
    render() {
        ...
        if (this.state.meetingHeaders !== undefined) {
            const sideBar = (
                <div className="relative w-full h-full bg-red-800 text-white p-4">
                    <div className="sticky top-36 grid grid-cols-1 sm:grid-cols-2 justify-items-center md:block text-center md:text-left">
                        {this.state.meetingHeaders.map((meetingHeader, index) => {
                            return (
                                <div key={index}>
                                    <button
                                        className="p-1 hover:bg-red-900"
                                        onClick={() => this.navigateToSlug(meetingHeader.getSlug())}
                                    >
                                        {meetingToName(meetingHeader.m_type)}{" "}
                                        {meetingHeader.date.getFullYear()}
                                    </button>
                                </div>
                            );
                        })}
                    ...
                    <div className="md:pl-4 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 justify-between">
                        <div className="px-4 md:pl-0 col-span-2 p-2 lg:col-span-3 mt-2 mb-8">
                            <div className="text-center grid">
                                {this.state.currentMeeting.date.getTime() >
                                    new Date().getTime() && (
                                    <div className="text-lg">Next up...</div>
                                )}
                                <div className="text-4xl font-semibold">
                                    {meetingToName(this.state.currentMeeting.m_type)}
                                </div>
                                ...
                                {this.state.currentRoles.length > 0 &&
                                    this.state.currentRoles[0].slug !== undefined && (
                                        <>
                                                ...
                                                {this.state.currentRoles.map((role, index) => {
                                                    ...
                                                    return (
                                                        <Collapse
                                                            ...
                                                            icon="arrow"
                                                        >
                                                            <Collapse.Title className="group-hover:bg-slate-300 text-xl font-semibold">
                                                                {role.name}
                                                            </Collapse.Title>
                                            ...
                                            <div className="md:max-w-lg w-full justify-self-center text-left border">
                                                {this.state.currentRoles.map((officer, index) => {
                                                    return (
                                                        ...
                                                        >
                                                            <Collapse.Title className="group-hover:bg-slate-300 text-xl font-semibold">
                                                                {officer.name}: {officer.role.name}
                                                            </Collapse.Title>
                                                            <Collapse.Content className="bg-slate-50">
                                                                <div>
                                                                    <b>Description: </b>
                                                                    <i>{officer.role.desc}</i>
                                                                </div>
                                                                <div>
                                                                    <b>Election Method: </b>
                                                                    <ElectionMethodModal
                                                                        text={methodName(
                                                                            officer.role.e_method,
                                                                        )}
                                                                        text_props="text-blue-700 hover:underline"
                                                                        method={
                                                                            officer.role.e_method
                                                                        }
                                                                    />
                                ...
                                {this.state.currentCandidates.length > 0 && (
                                    <>
                                        ...
                                        <div
                                            className={
                                                "grid grid-cols-1 justify-evenly place-items-center" +
                                                (this.state.currentCandidates.length > 1
                                                    ? " md:grid-cols-2" +
                                                          this.state.currentCandidates.length >
                                                      2
                                                        ? " lg:grid-cols-3"
                                                        : ""
                                                    : "")
                        ...
                        <div className="w-full h-full justify-self-end block">{sideBar}</div>
                    </div>
                </div>
            );
        } else {
