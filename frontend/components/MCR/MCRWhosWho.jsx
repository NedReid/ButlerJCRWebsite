...
class MCRWhosWho extends React.Component {
    constructor(props) {
        super(props);
        this.state = {whosWho: "waiting", editor: props.admin.democracy, editing: false, category: "mcr", saving: false, newCategoryText: ""}
    }

    async componentDidMount() {
        const res = await getWhosWhoOfCategory(this.state.category)
        const res1 = res.map((person) => {return new whosWhoModel(person)}).map((person) => {
