                        ...
                        {this.state.posts.map((post, index) => {
                            return (
                                <div
                                    key={post._id}
                                    className={
                                        "w-11/12 my-4 bg-white border-2 border-gray-400 rounded-lg transition-all ease-in-out duration-200 hover:translate-y-1 hover:brightness-90 hover:cursor-pointer" +
                                        (index >= 3 ? " lg:hidden xl:inline" : "")
                                    }
                                    onClick={() => this.goToPost(post._id)}
                                >
                                    <div className="p-2 bg-gray-200 col-span-2 font-semibold text-lg rounded-t-lg text-ellipsis whitespace-nowrap">
                                        {this.limitText(post.title, 32)}
                                    </div>
                                    <div className="p-2 col-span-2 bg-white text-sm rounded-b-lg">
                                        {this.limitText(
                                            post.post
                                                .replace("&nbsp;", " ")
                                                .replace("</p>", " ")
