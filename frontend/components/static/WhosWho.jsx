...
class WhosWho extends React.Component {
    ...
    async componentDidMount() {
        ...
        const res1 = res.map((person) => {
            return new whosWhoModel(person);
        });
        const res2 = res.map((person) => {
            return new whosWhoModel(person);
        });
        res1.sort((a, b) => {
    ...
    onChange = async () => {
        ...
        if (this.state.editing) {
            ...
            for (const np of newWhosWho) {
                let op = this.state.originalWhosWho.filter((p) => {
                    return np._id === p._id;
                });
                if (op.length === 0) {
                    console.log("Np", np);
            ...
            let res = await getWhosWhoOfCategory(this.state.category);
            const res1 = res.map((p) => {
                return new whosWhoModel(p);
            });
    ...
    newWhosWho = (subcategory) => {
        ...
        this.setState({
            whosWho: [
                ...this.state.whosWho.filter((person) => person.subcategory <= subcategory),
                newPerson,
                ...this.state.whosWho.filter((person) => person.subcategory > subcategory),
            ],
        });
    ...
    deleteWhosWho = (person) => {
        console.log(person);
        this.setState({
            whosWho: this.state.whosWho.filter((np) => {
                return np._id !== person._id;
            }),
        });
    };
    ...
    moveSubcategory = (personToMove, index) => {
        ...
        const lastOfNewCategoryIndex = newWhosWho.findLastIndex(
            (person) => person.subcategory === personToMove.subcategory,
        );
    ...
    render() {
        const subcategories =
            this.state.whosWho !== "waiting"
                ? [...new Set(this.state.whosWho.map((person) => person.subcategory))]
                : [];

        return (
            <div className="flex text-center flex-col w-full justify-center place-items-center relative">
                {this.state.whosWho !== "waiting" &&
                ...
                this.state.saving === false ? (
                    <>
                        {this.state.editor && (
                            <button
                                className={
                                    "z-30 btn btn-circle absolute top-0 right-0 my-2 swap swap-rotate " +
                        ...
                        {subcategories.map((subcategory, index) => {
                            const peopleOfSubcategory = this.state.whosWho.filter(
                            ...
                            return (
                                <div key={index} className="mb-8">
                                        ...
                                        {peopleOfSubcategory.map((person, index) => {
                                            ...
                                            return (
                                                <WhosWhoBlock
                                                    ...
                                                    updateWhosWho={this.updateWhosWho}
                                                    deleteWhosWho={this.deleteWhosWho}
                                    ...
                                    {this.state.editing && (
                                        <button
                                            className="mt-4 bg-amber-400 rounded p-2 transition hover:bg-amber-600"
                                            onClick={() => this.newWhosWho(subcategory)}
                                        >
                                            Add Person
                                        </button>
                                    )}
                            ...
                            <div className="bg-gray-100 border-2 border-gray-600 rounded-lg flex w-full p-2 max-w-xl">
                                <label className="my-auto">
                                    ...
                                    <input
                                        ...
                                        type="text"
                                        defaultValue={this.state.newCategoryText}
                                        onChange={this.updateNewCategoryText}
                                    />
                                </label>

                                <button
                                    className="ml-auto bg-gray-400 rounded p-2 transition hover:bg-gray-600"
