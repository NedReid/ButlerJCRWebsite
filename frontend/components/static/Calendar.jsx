...
class CalendarComponent extends React.Component {
    ...
    async componentDidMount() {
        ...
        const eventsModels = events.map((event) => {
    ...
    async componentDidUpdate(prevProps, prevState) {
        if (
            ...
            let eventsInRange = this.state.events.filter((event) => {
                return (
                    (event.startDate >= this.state.rangeStart &&
                        event.endDate <= this.state.rangeEnd) ||
                    (event.startDate >= this.state.rangeStart &&
                        event.startDate < this.state.rangeEnd) ||
                    (event.endDate > this.state.rangeStart &&
                        event.endDate <= this.state.rangeEnd) ||
                    (event.startDate < this.state.rangeStart &&
                        event.endDate >= this.state.rangeEnd)
                );
            });
            const idsInRange = eventsInRange.map((event) => {
                return event._id;
            });
            ...
            const idsToLoad = eventsToLoad.map((event) => {
                return event._id;
            });
    ...
    mouseEnter = (mouseEnterInfo) => {
        ...
        mouseEnterInfo.el.className =
            mouseEnterInfo.el.className + " hover:cursor-pointer hover:brightness-90";
    ...
    getEventRange = (fetchInfo, successCallback) => {
        ...
        const calendarEventSelection = this.state.events.map((ev) => {
            return {
                ...
                borderColor: calendarEventTypeColor(ev.category),
                backgroundColor: calendarEventTypeColor(ev.category),
                id: ev._id,
            };
        });
    ...
    dateText = (start, end, allDay) => {
        if (allDay) {
            if (end.valueOf() - start.valueOf() < 1000 * 60 * 60 * 47) {
                ...
                if (start.getMonth() === actualEnd.getMonth()) {
                    return (
                        ...
                        "-" +
                        actualEnd.toLocaleDateString("en-GB", { day: "numeric" }) +
                        " " +
                    ...
                    return (
                        ...
                        " - " +
                        actualEnd.toLocaleDateString("en-GB", {
                            day: "numeric",
                            month: "long",
                            year: "numeric",
                        })
            ...
            if (
                ...
                (start.getFullYear() === prevDayEnd.getFullYear() &&
                    start.getMonth() === prevDayEnd.getMonth() &&
                    start.getDate() === prevDayEnd.getDate() &&
                    prevDayEnd.getHours() < 6)
            ) {
                return (
    ...
    render() {
        ...
        if (this.state.events !== undefined) {
            return (
                <>
                    {this.props.admin.events && (
                        <div className="bg-slate-500 text-white w-full h-12 pl-2 flex">
                            <div className="w-full h-12 flex flex-grow items-center">
                                You have permission to edit calendar events.
                            </div>
                            <button
                                className={
                                    "flex-grow-0 flex-shrink-0 w-16 h-12 bg-slate-600 px-1 ml-1 transition hover:bg-slate-700"
                                }
                    ...
                    <div
                        ...
                        onMouseOver={this.tooltipMouseEnter}
                        onMouseLeave={this.tooltipMouseLeave}
                        className={
                            "w-40 bg-white absolute z-20 border-2 border-gray-400 hover:cursor-pointer hover:brightness-95" +
                            (this.state.tooltip.visible ? "" : " hidden")
                        }
                        onClick={this.tooltipEventClick}
                    >
                        <div className="flex w-full h-full">
                            <div
                                className="ml-2 mr-1 my-1 w-1"
                                style={{
                                    backgroundColor: calendarEventTypeColor(
                                        this.state.tooltip.visible
                                            ? this.state.tooltip.event.category
                                            : 0,
                                    ),
                                }}
                            >
                            ...
                            <div>
                                ...
                                <div className="text-sm flex items-center mb-1">
                                    <AiFillEnvironment className="font-bold mr-1" />{" "}
                                    {this.state.tooltip.visible
                                        ? this.state.tooltip.event.location
                                        : ""}
                                </div>
                                <div className="text-sm italic">
                                    {this.state.tooltip.visible
                                        ? this.state.tooltip.event.shortDescription
                                        : ""}
                                </div>
                            </div>
                        </div>
                            ...
                            <>
                                {this.state.loadedEventsInRange.map((event) => {
                                    return (
                                        <Collapse
                                            key={event._id}
                                            className="my-2 hover:bg-gray-100 active:bg-gray-200"
                                            icon="arrow"
                                            id={"desc" + event._id}
                                        >
                                            <input
                                            ...
                                            <Collapse.Title
                                                ...
                                                style={{
                                                    borderColor: calendarEventTypeColor(
                                                        event.category,
                                                    ),
                                                }}
                                            >
                                            ...
                                            <Collapse.Content className="bg-white border-l-4 border-gray-200">
                                                ...
                                                {event.link !== "" && (
                                                    <div className="flex items-center mb-1">
                                                        <AiOutlineLink className="font-bold mr-1" />{" "}
