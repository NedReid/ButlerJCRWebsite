            ...
            <div className="my-2 p-2 rounded border-2 border-amber-500">
                {this.state.addingMotion === false && (
                    <button
                        ...
                        onClick={() => {
                            this.setState({ addingMotion: true });
                        }}
                    >
                        Create Motion
                    </button>
                ...
                {this.state.meetingSelect !== "" && (
                    ...
                    >
                        ...
                        {this.state.motions
                            .filter((motion) => {
                                return motion.meeting === this.state.meetingSelect;
                            })
                            .map((motion, index) => {
                                return (
                                    <option
                ...
                {this.state.currentMotion !== undefined &&
                    this.state.motions.map((motion, index) => {
                        if (motion === this.state.currentMotion) {
                            return (
                                <CreateEditMotion
                                    closeTab={this.onUpdated}
                                    key={index}
                                    motion={this.state.currentMotion}
                                    meetings={this.state.meetings}
                                />
                            );
                        }
