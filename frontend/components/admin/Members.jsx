    ...
    render() {
        return (
            <div className="my-2 p-2 rounded border-2 border-amber-500">
                ...
                <Textarea
                    onChange={(event) => this.setState({ memberList: event.target.value })}
                    placeholder="CIS codes"
                ></Textarea>
                <div>Add all the JCR Members. CIS Codes separated by line breaks</div>
                <label>
                    {" "}
                    Year of membership Expiry (YY):
                    <input
                        className=" ml-2 mb-2 rounded border-2 border-slate-500"
                        name="noTickets"
                        type="number"
                        onChange={(event) =>
                            this.setState({ expiryYear: parseInt(event.target.value) })
                        }
                        onKeyPress={(event) => {
                ...
                <div className="flex">
                    <Checkbox
                        onChange={() => this.setState({ remove: !this.state.remove })}
                        className="mr-2"
                    ></Checkbox>
