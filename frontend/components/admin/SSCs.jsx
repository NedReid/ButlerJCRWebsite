    ...
    render() {
        return <div className="my-2 p-2 rounded border-2 border-amber-500">
            ...
            <select defaultValue="" onChange={this.changeTargetType} className="select select-bordered w-full max-w-xs">
                ...
                <option value={SSCEnum.society} key="SSCType">Societies</option>
                <option value={SSCEnum.sport} key="SSCType">Sports</option>
                <option value={SSCEnum.committee} key="SSCType">Committees</option>
            </select>
            <br/>
            {this.state.typeSelect !== "" && <select
                defaultValue="" onChange={this.editSSC} key={this.state.typeSelect} className="select select-bordered w-full max-w-xs">
                <option value="">Select {getSSCNames(this.state.typeSelect)}</option>
                {this.state.SSCs.filter((ssc) => {return ssc.type === this.state.typeSelect}).map((ssc, index) => {
                    return (
                        <option value={ssc._id} key={this.typeSelect + "_" + index}>{ssc.name}</option>

                    );
                })}
            </select>}
            <br/>
            {this.state.currentSSC !== undefined && this.state.SSCs.map((ssc, index) =>{
                if(ssc === this.state.currentSSC) {
                    return <CreateEditSSC closeTab={this.onUpdated} key={index} SSC={this.state.currentSSC}/>
                }
            })}
        </div>
