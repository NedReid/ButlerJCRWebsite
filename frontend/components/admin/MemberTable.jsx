...
class MemberTable extends React.Component {
    constructor(props) {
        ...
        super(props);
        this.state = {members: undefined, toDelete: undefined, descending: false, sortby: "", page: 1};
    }
    ...
    deleteButton = async () => {
        ...
        await this.setState({members: this.state.members.filter(member => member._id !== this.state.toDelete._id)})
    ...
    sortMembers = async (category) => {
        ...
        let newMembers = this.state.members.sort((a,b) => (a[category] > b[category]) ? 1 * multiplier : ((b[category] > a[category]) ? -1 * multiplier : 0))
    ...
    render() {
        ...
        const icons = "text-xl align-middle justify-self-center text-gray-500 hover:text-gray-800 hover:cursor-pointer ml-2"
        const pages = this.state.members === undefined? 1 : Math.ceil(this.state.members.length / 20)
        return (this.state.members !== undefined ? <div className="grid">
            <Table compact className="mx-4 mt-2">
                <Table.Head>
                    <span className="flex">
                        CIS Code
                        {((this.state.sortby !== "username") || !this.state.descending)?
                            <AiOutlineSortAscending className={icons} onClick={async () => await this.sortMembers("username")}/> :
                            <AiOutlineSortDescending className={icons} onClick={async () => await this.sortMembers("username")}/>}
                    </span>
                    <span className="flex">Transaction
                    {((this.state.sortby !== "transaction") || !this.state.descending)?
                        <AiOutlineSortAscending className={icons} onClick={async () => await this.sortMembers("transaction")}/> :
                        <AiOutlineSortDescending className={icons} onClick={async () => await this.sortMembers("transaction")}/>}
                    </span>
                    <span className="flex">Expiry Date
                    {((this.state.sortby !== "expiry") || !this.state.descending)?
                        <AiOutlineSortAscending className={icons} onClick={async () => await this.sortMembers("expiry")}/> :
                        <AiOutlineSortDescending className={icons} onClick={async () => await this.sortMembers("expiry")}/>}
                    </span>
                    <span>Delete</span>
                </Table.Head>
                <Table.Body>
                    {this.state.members.slice((this.state.page - 1) * 20, this.state.page * 20).map((member, index) => {
                        return <Table.Row key={member._id}>
            ...
            <Pagination className=" flex justify-center">
                ...
                <div className="modal-box">
                    ...
                    <p className="py-4">Are you sure you want to delete {this.state.toDelete? this.state.toDelete.username :"this user"} as a JCR Member?</p>
                    <div className="modal-action">
                        <label onClick={this.deleteButton} htmlFor="my-modal" className="btn">Delete</label>
