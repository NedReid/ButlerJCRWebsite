...
class CreateEditMotion extends React.Component {
    constructor(props) {
        ...
        super(props);
        this.state = {motion: new motionModel(), meetings: this.props.meetings, currentYear: (new Date().getFullYear())};
        ...
        return <> <div className="my-2 p-2 rounded border-2 border-amber-500 flex flex-col">
            ...
            <label>Motion Name:
                <input className=" ml-2 mb-2 rounded border-2 border-slate-500"
                       name="name" type="text" defaultValue={this.state.motion.name} onChange={(event) => this.handleChange(event, "name")}/>
            </label>
            ...
            <div className="p-1 rounded border-2 border-slate-500">
                <TextEditor initialValue={this.state.motion.notes} onUpdate={this.handleNotes} />
            </div>

            <label>This JCR Believes:</label>
            <div className="p-1 rounded border-2 border-slate-500">
                <TextEditor initialValue={this.state.motion.notes} onUpdate={this.handleBelieves} />
            </div>

            <label>This JCR Resolves:</label>
            <div className="p-1 rounded border-2 border-slate-500">
                <TextEditor initialValue={this.state.motion.notes} onUpdate={this.handleResolves} />
            </div>

            <label>Proposer:
                <input className=" ml-2 mb-2 rounded border-2 border-slate-500"
                       name="proposer" type="text" defaultValue={this.state.motion.proposer} onChange={(event) => this.handleChange(event, "proposer")}/>
            </label>

            <label>Seconder:
                <input className=" ml-2 mb-2 rounded border-2 border-slate-500"
                       name="seconder" type="text" defaultValue={this.state.motion.seconder} onChange={(event) => this.handleChange(event, "seconder")}/>
            </label>

            <label> Ended:</label>
            <span>
                <label><input defaultChecked={this.state.motion.ended === false}  onChange={(event) => this.handleBoolChange(event, "ended")} type="radio" name={this.state.motion._id + "ended"} value={false}/>
                False</label>
                <label className="ml-2"><input defaultChecked={this.state.motion.ended === true} onChange={(event) => this.handleBoolChange(event, "ended")} type="radio" name={this.state.motion._id + "ended"} value={true}/>
                True</label>
            </span>

            <label> Passed:</label>
            <span>
                <label><input defaultChecked={this.state.motion.passed === false}  onChange={(event) => this.handleBoolChange(event, "passed")} type="radio" name={this.state.motion._id + "passed"} value={false}/>
                False</label>
                <label className="ml-2"><input defaultChecked={this.state.motion.passed === true} onChange={(event) => this.handleBoolChange(event, "passed")} type="radio" name={this.state.motion._id + "passed"} value={true}/>
                True</label>
            </span>
