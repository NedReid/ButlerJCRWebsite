    ...
    render() {
        if (this.state.visibility !== undefined) {
            return <div className="my-2 p-2 rounded border-2 border-amber-500">
                ...
                <br/>
                <select defaultValue={this.state.visibility} onChange={this.updateVisibility} key="fVis" className="select select-bordered w-full max-w-xs">
                    <option value={"visible"} key={"fVis_visible"}>Visible to all</option>
                    <option value={"invisible"} key={"fVis_invisible"}>Invisible to all</option>
                    <option value={"preview"} key={"fVis_preview"}>Preview (Visible to you)</option>
                </select>
                <br/>
                <button className="bg-amber-400 rounded p-2 transition hover:bg-amber-600" onClick={this.submitButton}>Update Visibility</button>
                {this.state.complete && <div className="italic text-sm">Updated to {this.state.visibility}</div>}
