...
class PostCategories extends React.Component {
    ...
    render() {
        return <div className="my-2 p-2 rounded border-2 border-amber-500">
            ...
            <button className="bg-amber-400 rounded p-2 transition hover:bg-amber-600" onClick={() => {this.setState({addingCategory: true})}}>Add Category</button>
            }
            {this.state.addingCategory === true && <>
                <button className="bg-amber-400 rounded p-2 transition hover:bg-amber-600" onClick={() => {this.setState({addingCategory: false})}}>Cancel</button>
            ...
            <select defaultValue="" onChange={this.editPostCategory} key="postCategories" className="select select-bordered w-full max-w-xs">
            ...
            <br/>
            {this.state.currentCategory !== undefined && this.state.postCategories.map((category, index) =>{
                if(category === this.state.currentCategory) {
                    console.log("AGHGG")
                    return <CreateEditPostCategory closeTab={this.onUpdated} key={index} postCategory={this.state.currentCategory}/>
                }
            })}
        </div>
    }
