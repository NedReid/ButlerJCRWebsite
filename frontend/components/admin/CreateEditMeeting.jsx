        ...
        return <> <div className="my-2 p-2 rounded border-2 border-amber-500 flex flex-col">
            ...
            <label> Meeting Date:
                <DateTimePicker disableCalendar={true} disableClock={true} className="ml-2 mb-2"
                                ...
                                name="ticketReleaseDate" value={this.state.meeting.date} onChange={this.handleMeetingDate}
                />
            </label>

            <label>Meeting Type:</label>
            <select name="m_type" defaultValue={this.state.meeting.m_type} onChange={(event) => this.handleNumChange(event, "m_type")} className="select select-bordered w-full max-w-xs">
                {Object.values(meetingEnum).map((meeting) => {
                    return <option value={meeting} key="m_type">{meetingToName(meeting)}</option>
                })}
            </select>

            <label>Meeting Page:</label>
            <div className="p-1 rounded border-2 border-slate-500">
                <TextEditor initialValue={this.state.meeting.page} onUpdate={this.handlePage} />
            </div>

            <label> Visible:</label>
            <span>
                <label><input defaultChecked={this.state.meeting.visible === false}  onChange={(event) => this.handleBoolChange(event, "visible")} type="radio" name={this.state.meeting._id + "visible"} value={false}/>
                False</label>
                <label className="ml-2"><input defaultChecked={this.state.meeting.visible === true} onChange={(event) => this.handleBoolChange(event, "visible")} type="radio" name={this.state.meeting._id + "visible"} value={true}/>
                True</label>
            </span>
