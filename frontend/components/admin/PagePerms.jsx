...
class PagePerms extends React.Component {
    ...
    render() {
        return <div className="my-2 p-2 rounded border-2 border-amber-500">
            ...
            <select defaultValue="" onChange={this.editPagePerm} key="pagePerms" className="select select-bordered w-full max-w-xs">
                <option value="">Select Page</option>
                {this.state.pagePerms.map((page, index) => {
                    return (
                        <option value={page._id} key={"page_" + index}>{page.page}</option>

                    );
                })}
            </select>
            <br/>
            {this.state.currentPerm !== undefined && this.state.pagePerms.map((page, index) =>{
                if(page === this.state.currentPerm) {
                    return <CreateEditPagePerm closeTab={this.onUpdated} key={index} pagePerm={this.state.currentPerm}/>
                }
            })}
        </div>
    }
