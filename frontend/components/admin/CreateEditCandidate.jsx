...
class CreateEditCandidate extends React.Component {
    constructor(props) {
        ...
        console.log("ROLES")
        this.state = {candidate: new candidateModel(), meetings: this.props.meetings, currentYear: (new Date().getFullYear()), currentCategory: roleCategoryEnum.other};
        ...
        return <> <div className="my-2 p-2 rounded border-2 border-amber-500 flex flex-col">
            ...
            <label>Candidate Name:
                <input className=" ml-2 mb-2 rounded border-2 border-slate-500"
                       name="name" type="text" defaultValue={this.state.candidate.name} onChange={(event) => this.handleChange(event, "name")}/>
            </label>
            ...
            <label>Candidate Video (Leave Blank to omit. Youtube videos pls!):
                <input className=" ml-2 mb-2 rounded border-2 border-slate-500"
                       name="video" type="text" defaultValue={this.state.candidate.video} onChange={(event) => this.handleChange(event, "video")}/>
            </label>

            <label>Candidate Poster:
                <input className=" ml-2 mb-2 rounded border-2 border-slate-500"
                       name="poster" type="file" accept="image/*,application/pdf" onChange={(event) => this.handleImageChange(event, "poster")}/>
                {this.state.candidate.poster !== "" && (
                    this.state.candidate.poster.startsWith("data:application/pdf")?
                        <div/>
                    :
                        <img className="h-24 object-contain" src={this.state.candidate.poster}/>
                            )}
            </label>

            <label>Candidate Promotional Image:
                <input className=" ml-2 mb-2 rounded border-2 border-slate-500"
                       name="promotionalImage" type="file" accept="image/*,application/pdf" onChange={(event) => this.handleImageChange(event, "promotionalImage")}/>
                {this.state.candidate.promotionalImage !== "" && (
                    this.state.candidate.promotionalImage.startsWith("data:application/pdf")?
                        <div/>
                        :
                        <img className="h-24 object-contain" src={this.state.candidate.promotionalImage}/>
                )}            </label>

            <label>Candidate Meeting:</label>
            <label> Year:
                <input className=" ml-2 mb-2 rounded border-2 border-slate-500" name="election_year"
                       type="number" defaultValue={this.state.currentYear} onChange={this.changeYear}
                       onKeyPress={(event) => {
            ...
            <select name="category" defaultValue={this.state.currentCategory} onChange={this.changeCategoryType} className="select select-bordered w-full max-w-xs">
            ...
            <select key={this.state.currentCategory} name="category" defaultValue={this.state.candidate.role} onChange={(event) => this.handleChange(event, "role")} className="select select-bordered w-full max-w-xs">
                <option value="">Select Officer</option>
                {this.props.roles.filter((role) => {return role.category === this.state.currentCategory}).map((role, index) => {
                    return (
                        <option value={role._id} key={this.currentCategory + "_" + index}>{role.name}</option>
            ...
            <label> Ended:</label>
            <span>
                <label><input defaultChecked={this.state.candidate.ended === false}  onChange={(event) => this.handleBoolChange(event, "ended")} type="radio" name={this.state.candidate._id + "ended"} value={false}/>
                False</label>
                <label className="ml-2"><input defaultChecked={this.state.candidate.ended === true} onChange={(event) => this.handleBoolChange(event, "ended")} type="radio" name={this.state.candidate._id + "ended"} value={true}/>
                True</label>
            </span>

            <label> Passed:</label>
            <span>
                <label><input defaultChecked={this.state.candidate.passed === false}  onChange={(event) => this.handleBoolChange(event, "passed")} type="radio" name={this.state.candidate._id + "passed"} value={false}/>
                False</label>
                <label className="ml-2"><input defaultChecked={this.state.candidate.passed === true} onChange={(event) => this.handleBoolChange(event, "passed")} type="radio" name={this.state.candidate._id + "passed"} value={true}/>
                True</label>
            </span>
