...
class Officers extends React.Component {
    constructor(props) {
        super(props);
        this.state = {addingOfficer: false, officers: [], currentOfficer: undefined, typeSelect:"", roleSelect:undefined};
    ...
    render() {
        return <div className="my-2 p-2 rounded border-2 border-amber-500">
            ...
            <button className="bg-amber-400 rounded p-2 transition hover:bg-amber-600" onClick={() => {this.setState({addingOfficer: true})}}>Create Officer</button>
            }
            {this.state.addingOfficer === true && <>
                <button className="bg-amber-400 rounded p-2 transition hover:bg-amber-600" onClick={() => {this.setState({addingOfficer: false})}}>Cancel</button>
            ...
            {this.state.typeSelect !== "" && <select
                defaultValue="" onChange={this.changeRoleType} key={this.state.typeSelect} className="select select-bordered w-full max-w-xs">
            ...
            <br/>
            {this.state.roleSelect !== undefined && <select
                defaultValue="" onChange={this.editOfficer} key={this.state.roleSelect._id} className="select select-bordered w-full max-w-xs">
                <option value="">Select Officer</option>
                {this.state.officers.filter((officer) => {return officer.role === this.state.roleSelect._id}).map((officer, index) => {
                    return (
                        <option value={officer._id} key={this.typeSelect + "_" + index}>{officer.name}</option>

                    );
                })}
            </select>}
            <br/>
            {this.state.currentOfficer !== undefined && this.state.officers.map((officer, index) =>{
                if(officer === this.state.currentOfficer) {
                    return <CreateEditOfficer closeTab={this.onUpdated} key={index} officer={this.state.currentOfficer} roles={this.state.roles}/>
                }
            })}
        </div>
    }
