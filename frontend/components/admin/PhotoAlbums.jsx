...
class PhotoAlbums extends React.Component {
    ...
    getYearsFromDates = (photoAlbums) => {
        const years = photoAlbums
            .map((photoAlbum) => new Date(photoAlbum.date))
            .sort((a, b) => b - a)
            .map((albumDate) => {
            ...
            <div className="my-2 p-2 rounded border-2 border-amber-500">
                ...
                <select
                    ...
                    value={
                        this.state.currentDateFilter === null
                            ? null
                            : this.state.dateRanges.indexOf(this.state.currentDateFilter)
                    }
                    onChange={(event) =>
                        this.setState({
                            currentDateFilter:
                                event.target.value === null
                                    ? null
                                    : this.state.dateRanges[event.target.value],
                        })
                    }
                    key="photoAlbumYear"
                ...
                >
                    ...
                    {this.state.dateRanges.map((dateRange, index) => {
                        return (
                            <option value={index} key={"dateRange_" + index}>
                                {dateRange.name}
                            </option>
                        );
                    })}
                    ...
                    >
                        ...
                        {this.state.photoAlbums
                            .filter((photoAlbum) => {
                                ...
                                return (
                                    albumDate < this.state.currentDateFilter.endDate &&
                                    albumDate >= this.state.currentDateFilter.startDate
                                );
                            })
                            .map((photoAlbum, index) => {
                                return (
                                    <option value={photoAlbum._id} key={"photoAlbum_" + index}>
                                        {photoAlbum.name}
                                    </option>
                                );
                            })}
                ...
                <br />
                {this.state.currentPhotoAlbum !== undefined &&
                        ...
                        if (photoAlbum === this.state.currentPhotoAlbum) {
                            return (
                                <EditPhotoAlbum
                                    ...
                                    key={index}
                                    photoAlbum={this.state.currentPhotoAlbum}
                                />
                            );
                        }
                    })}
