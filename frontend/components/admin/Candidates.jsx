...
class Candidates extends React.Component {
    ...
    render() {
        return <div className="my-2 p-2 rounded border-2 border-amber-500">
            ...
            <button className="bg-amber-400 rounded p-2 transition hover:bg-amber-600" onClick={() => {this.setState({addingCandidate: true})}}>Create Candidate</button>
            }
            {this.state.addingCandidate === true && <>
                <button className="bg-amber-400 rounded p-2 transition hover:bg-amber-600" onClick={() => {this.setState({addingCandidate: false})}}>Cancel</button>
                <CreateEditCandidate closeTab={this.onCreated} meetings={this.state.meetings} roles={this.state.roles}/>
            </>}
            <br/>
            ...
            {this.state.meetingSelect !== "" && <select
                defaultValue="" onChange={this.editCandidate} key={this.state.meetingSelect} className="select select-bordered w-full max-w-xs">
                <option value="">Select Candidate</option>
                {this.state.candidates.filter((candidate) => {return candidate.meeting === this.state.meetingSelect}).map((candidate, index) => {
                    return (
                        <option value={candidate._id} key={this.meetingSelect + "_" + index}>{candidate.name}</option>

                    );
                })}
            </select>}
            <br/>
            {this.state.currentCandidate !== undefined && this.state.candidates.map((candidate, index) =>{
                if(candidate === this.state.currentCandidate) {
                    return <CreateEditCandidate closeTab={this.onUpdated} key={index} candidate={this.state.currentCandidate} meetings={this.state.meetings} roles={this.state.roles}/>
                }
            })}
        </div>
    }
