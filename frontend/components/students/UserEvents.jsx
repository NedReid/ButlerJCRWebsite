                    ...
                    <>
                        {this.state.events.map((event) => {
                            ...
                            return (
                                <div
                                    key={event._id}
                                    className="my-2 p-2 rounded border-2 border-amber-500 collapse collapse-arrow"
                                >
                                    ...
                                    <div className="collapse-content text-black">
                                        ...
                                        <hr />
                                        <div>
                                            Ticket release: {event.ticketReleaseDate.toString()}
                                        </div>
                                        <div>
                                            Ticket deadline:{" "}
                                            {event.ticketReleaseDeadline.toString()}
                                        </div>
                                    ...
                                    {Date.parse(event.ticketReleaseDate) < Date.now() &&
                                        Date.now() < Date.parse(event.ticketReleaseDeadline) && (
                                            <div>
                                                <button
                                                    className="bg-amber-400 rounded p-2 transition hover:bg-amber-600"
                                                    onClick={() => this.navigate(event._id)}
                                                >
                                                    Book Tickets
                                                </button>
                                            </div>
