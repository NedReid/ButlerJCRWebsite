    ...
    render() {
            return this.state.postCategories.length > 0 ?
                <div className="my-4 p-2">
                    ...
                    <button className="bg-amber-400 rounded p-2 transition hover:bg-amber-600" onClick={() => {
                    ...
                    {this.state.addingPost === true && <>
                        ...
                        <EditPost postCategories={this.state.postCategories} closeTab={this.onCreated}/>
                    </>}
                    <Divider/>
                    <div className="text-2xl font-semibold">Edit Current Post:</div>
                    <br/>

                    <select defaultValue="" onChange={this.editPost} key="posts"
                            className="select select-bordered w-full max-w-xs">
                        <option value="">Select Page</option>
                        {this.state.posts.map((post, index) => {
                            return (
                                <option value={post._id} key={"post_" + index}>{post.title}</option>

                            );
                        })}
                    </select>
                    <br/>
                    {this.state.currentPost !== undefined && this.state.posts.map((page, index) => {
                        if (page._id === this.state.currentPost._id) {
                            return <EditPost postCategories={this.state.postCategories} closeTab={this.onUpdated} key={index} post={this.state.currentPost}/>
                        }
                    })}
                </div>
                :
