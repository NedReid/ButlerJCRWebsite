...
export const createFAQ = async (q) => {
    const response = await axios.post("/api/admin/createFAQ", {
        question: q.question,
        answer: q.answer,
        order: q.order,
        category: q.category,
...
export const createDocument = async (document, file) => {
    const response = await axios.post("/api/admin/createDocument", {
        document: document,
        file: file,
    });
    if (response.status === 201) {
...
export const updateDocument = async (document, file) => {
    const response = await axios.post("/api/admin/updateDocument", {
        document: document,
        file: file,
    });
    if (response.status === 200) {
