...
export const getFolderAddresses = async (folder) => {
    const response = await axios.get("/api/static/getFolderAddresses", {
...
export const updatePageEditables = async (page, editables) => {
    const response = await axios.post("/api/static/updatePageEditables", {
        page: page,
        editables: editables,
    });
    if (response.status === 200) {
...
export const getPostsOfTypes = async (types, number) => {
    console.log(types);
    const response = await axios.get("/api/getPostsOfTypes", {
        params: { types: types, number: number },
    });
    if (response.status === 200) {
        return response.data;
    }
...
export const getCalendarEventSelection = async (selection) => {
    const response = await axios.get("/api/getCalendarEventSelection", {
...
export const getWhosWhoOfCategory = async (category) => {
    const response = await axios.get("/api/getWhosWhoOfCategory", {
