...
export const staticRoutes = async (app, auth, db) => {
    ...
    app.get("/api/static/getPageEditables", async function (req, res) {
        ...
        editables = await Promise.all(
            await editables.map(async (ed) => {
            ...
            if (pagePerms.editors.includes(webToken.username)) {
                await req.body.editables.map(async (ed) => {
                    if (ed.page === req.body.page) {
                        let edEnt = await db.editables.findOneAsync({
                            page: req.body.page,
                            name: ed.name,
                            _id: ed._id,
                        });
    ...
    app.get("/api/static/getAlbums", async function (req, res) {
        ...
        for (const album of albums) {
            ...
            fs.mkdirSync(path.join(__dirname, "../../files/albumsPreview/" + album), {
                recursive: true,
            });
            ...
            for (const photo of photos) {
                ...
                const outputPath = path.join(
                    __dirname,
                    "../../files/albumsPreview/" + album + "/" + photo + ".webp",
                );
            ...
            let previews = fs.readdirSync(
                path.join(__dirname, "../../files/albumsPreview/" + album),
            );
