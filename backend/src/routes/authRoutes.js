...
export const authRoutes = (app, auth, db) => {
        ...
        try {
            ...
            if (count === 0) {
                const newUser = await db.users.insertAsync({
                    ...
                    password: hash,
                    registered: auth.vToken({ username: req.body.username }),
                });
                console.log("added");
    ...
    app.post("/api/submitNewPassword", async function (req, res) {
        ...
        try {
            ...
            const user = await db.users.findOneAsync({
                username: webToken.username,
                passwordReset: req.body.token,
            });
            if (user !== null) {
    ...
    app.get("/api/isLoggedIn", async function (req, res) {
        ...
        try {
            ...
            const user = await db.users.findOneAsync({
                username: webToken === undefined ? "" : webToken.username,
            });
            const admin = await db.admins.findOneAsync({
                username: webToken === undefined ? "" : webToken.username,
            });
            ...
            if (
                freshersVisible !== null &&
                (freshersVisible.value === "visible" ||
                    (freshersVisible.value === "preview" &&
                        admin !== null &&
                        admin.freshers === true))
            ) {
                resp.freshers = true;
    ...
    app.get("/api/getUserByResetToken", async function (req, res) {
        ...
        try {
            ...
            const user = await db.users.findOneAsync({
                username: webToken.username,
                passwordReset: req.query.resetToken,
            });
            let resp = {
                username: false,
            };
