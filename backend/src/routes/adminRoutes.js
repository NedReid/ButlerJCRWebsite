...
export const adminRoutes = async (app, auth, db, __dirname) => {
    ...
    app.get("/api/admin/getMotions", async function (req, res) {
        if (res.locals.adminUser.democracy === true) {
            ...
            motions = await Promise.all(
                await motions.map(async (motion) => {
                    ...
                    if (typeof motion.believes === "string" || motion.believes instanceof String) {
                        motion.believes = await retrieveRichText(
                            motion.believes,
                            "motions_believes",
                        );
                    }
                    if (typeof motion.resolves === "string" || motion.resolves instanceof String) {
                        motion.resolves = await retrieveRichText(
                            motion.resolves,
                            "motions_resolves",
                        );
                    }
    ...
    app.post("/api/admin/updateMotion", async function (req, res) {
        if (res.locals.adminUser.democracy === true) {
            ...
            req.body.believes = await parseRichText(
                req.body.believes,
                req.body._id,
                "motion_believes",
            );
            req.body.resolves = await parseRichText(
                req.body.resolves,
                req.body._id,
                "motion_resolves",
            );
        ...
        if (res.locals.adminUser.democracy === true) {
            ...
            candidates = await Promise.all(
                await candidates.map(async (candidate) => {
                    if (
                        typeof candidate.manifesto === "string" ||
                        candidate.manifesto instanceof String
                    ) {
                        candidate.manifesto = await retrieveRichText(
                            candidate.manifesto,
                            "candidates",
                        );
                    }
                    candidate.poster = await retrieveImageFile(candidate.poster);
                    candidate.promotionalImage = await retrieveImageFile(
                        candidate.promotionalImage,
                    );
                    return candidate;
                }),
            );
    ...
    app.post("/api/admin/createCandidate", async function (req, res) {
        if (res.locals.adminUser.democracy === true) {
            ...
            newDb.promotionalImage = await exportImageFile(
                promotionalImage,
                true,
                "promotionalImage",
                newDb._id,
    ...
    app.post("/api/admin/updateCandidate", async function (req, res) {
        if (res.locals.adminUser.democracy === true) {
            req.body.manifesto = await parseRichText(
                req.body.manifesto,
                req.body._id,
                "candidates",
            );
            req.body.poster = await exportImageFile(
                req.body.poster,
                true,
                "poster",
                req.body._id,
            ...
            req.body.promotionalImage = await exportImageFile(
                req.body.promotionalImage,
                true,
                "promotionalImage",
                req.body._id,
            ...
            if (req.body.remove) {
                ...
                for (const member of membersToAdd) {
                    await db.members.insertAsync({
                        ...
                        transaction: "added manually by " + res.locals.adminUser.username,
                    });
                }
            }
    ...
    app.post("/api/admin/updateCalendarEvent", async function (req, res) {
        if (res.locals.adminUser.events === true) {
            // console.log(req.body);
            req.body.description = await parseRichText(
    ...
    app.post("/api/admin/updateWhosWho", async function (req, res) {
        req.body.photo = await exportImageFile(
            req.body.photo,
            false,
            "photo",
            req.body._id,
