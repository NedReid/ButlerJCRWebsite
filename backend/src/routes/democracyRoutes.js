...
export const democracyRoutes = async (app, auth, db) => {
                ...
                if (value === meetingEnum.none || value === meetingEnum.emergency) {
                    ...
                    if (sl.length === 4) {
                        return meetings.find((meeting) => {
                            ...
                            return (
                                meeting.m_type === value &&
                                date.getDate().toString() === sl[1] &&
                                date.getMonth().toString() === sl[2] &&
                                date.getFullYear().toString() === sl[3]
                            );
                        });
                    }
                } else {
                    ...
                    if (sl.length === 3) {
                        return meetings.find((meeting) => {
                            const date = new Date(meeting.date);
                            return (
                                meeting.m_type === value && date.getFullYear().toString() === sl[2]
                            );
                        });
                    }
                }
    ...
    const getApplicableRoles = async (meeting) => {
        ...
        if (new Date().getTime() > dayAfter.getTime()) {
            ...
            let electedOfficers = currentOfficers.filter((officer) => {
                return (
                    officer.election_meeting === meeting.m_type &&
                    officer.election_year === new Date(meeting.date).getFullYear()
                );
            });
            ...
            let filteredRoles = roles.filter((role) => {
                return (
                    (role.e_meeting === meeting.m_type ||
                        currentOfficers.filter((officer) => officer.role === role._id).length <
                            role.e_seats) &&
    ...
    const checkOfficer = async (req, webToken) => {
        ...
        if (foundOfficers !== null) {
            return foundOfficers
                .filter((officer) => officer.current)
    ...
    app.get("/api/democracy/getRoleHeaders", async function (req, res) {
        ...
        if (!(await checkAdmin(req, webToken))) {
            ...
            roles = roles.filter(
                (role) => role.visible === true || officerPerms.includes(role._id),
            );
        }
    ...
    app.get("/api/democracy/getOfficerHeaders", async function (req, res) {
        ...
        officers = await Promise.all(
            await officers.map(async (officer) => {
                officer.page = "beans";
                return officer;
            }),
        );
    ...
    app.get("/api/democracy/getMotionHeaders", async function (req, res) {
        ...
        if (!(await checkAdmin(req, webToken))) {
            motions = await Promise.all(
                await motions.filter(async (motion) => {
    ...
    app.get("/api/democracy/getCandidateHeaders", async function (req, res) {
        ...
        if (!(await checkAdmin(req, webToken))) {
            candidates = await Promise.all(
                await candidates.filter(async (candidate) => {
